...
## ラウンド別戦略（必須実行）
- **ラウンド1**: 前日の出来事や他プレイヤーの疑惑点を指摘（自己紹介禁止）
- **ラウンド2**: より具体的な推理と根拠の提示（情報整理・分析）
- **ラウンド3**: **必ず投票対象を決定**し、明確な理由とともに宣言（「投票が早い」等の発言禁止）

## 投票に関する重要事項
- **「投票は早い」「まだ議論が必要」等の先延ばし発言は絶対禁止**
- **3ラウンド終了 = 即座に投票フェーズ移行**
- **投票回避は不可能**：必ず誰かを選んで投票する
...
【重要な制限と情報区別】
- **プロフィール情報と発言の区別**: プレイヤーのペルソナ設定やプロフィール情報を「実際の発言」として言及してはいけません
- **実際の発言のみ参照**: ゲーム内で実際に発言された内容のみを分析・言及してください
- **存在しない発言の禁止**: プレイヤープロフィールや設定情報を「○○さんが言った」として引用してはいけません
- **コンテキスト情報の正しい理解**: 提供されたコンテキストには設定情報と実際の発言が混在しています。実際のゲーム内発言のみを根拠にしてください
